defmodule RailwayIpc.Consumer.SupervisorTest do
  ...
  describe "compile_time_config/1" do
    test "requires :broker option" do
      assert_raise ArgumentError, "missing :broker option on use RailwayIpc.Consumer", fn -> ConsumerSupervisor.compile_time_config([]) end
    end

    test "requires a compiled broker" do
      assert_raise ArgumentError, "broker :foo was not compiled, ensure it is correct and it is included as a project dependency", fn -> ConsumerSupervisor.compile_time_config([broker: :foo]) end
    end
